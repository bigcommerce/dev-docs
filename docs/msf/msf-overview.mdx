...
Consult the [Channels API documentation](/docs/rest-management/channels) for more information on valid combinations of `type` and `platform`. For example, a BigCommerce-hosted Stencil storefront has a type of `storefront` and a platform of `bigcommerce`.
