...
To work with the improved v2, see our article on [Next.js + BigCommerce](/api-docs/storefronts/nextjs-commerce).
</Callout>
...
- `STORE_TOKEN|STORE_CLIENT`: For instructions on generating store-level API credentials, visit [Creating store-level API credentials](/api-docs/getting-started/api-accounts#creating-store-level-api-credentials).
