...
There is now a clear separation of [options that define variants](/docs/store-operations/catalog#variant-options) versus [options that are modifiers of a variant](/api-docs/store-management/catalog/catalog-overview#modifier-options). This simplifies the creation and management of variant prices and modifier adjusters and removes the need to use complex rules, in all but the most extreme cases.
