...
There is now a clear separation of [options that define variants](/api-docs/store-management/catalog/catalog-overview#variant-options) versus [options that are modifiers of a variant](/api-docs/catalog/products-overview#products-overview_modifier-options). This simplifies the creation and management of variant prices and modifier adjusters and removes the need to use complex rules, in all but the most extreme cases.
...
V3 includes an endpoint for retrieving a [Catalog Tree](/api-reference/catalog/catalog-api).
