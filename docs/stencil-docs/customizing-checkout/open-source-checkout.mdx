...
You can create unique shopping experiences that fit the look and feel of a store's brand using custom checkouts. This article will outline how to package a custom checkout file and install a custom checkout using the control panel.
...
2. Start webpack in watch mode and launch dev server.
...
You'll need to host the custom checkout file online so the store can serve it. You can use a hosting service, such as [Amazon S3](https://aws.amazon.com/s3/). Using an external host will allow you the freedom to automate the build process if you wish and push updates automatically from your local machine. Or you can host it on your store using WebDAV.
