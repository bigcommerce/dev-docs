...
By default, wallet buttons are not enabled on the PDP. To add wallet buttons, you must first prepare your theme by adding a checkbox to Page Builder. The checkbox allows the retailer to control the display of wallet buttons on the PDP on your store. Perform the following steps to add the checkbox:
...
2. To create a checkbox for the retailer to toggle the feature on and off in Page Builder, add the following code to the Products section of `schema.json`:
...
2. To add a Page Builder dialog for the retailer to configure the number of visible wallet buttons, add the following code to the Payments section of `schema.json`:
...
Adding a sorting features allows retailers to choose the order in which wallet buttons appear to shoppers. To add a sorting feature, perform the following steps:
