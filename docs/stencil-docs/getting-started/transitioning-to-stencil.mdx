...
Stencil improves the overall storefront experience for retailers and their customers. With enhancements in speed, security, and updates to your themes, this modern framework provides a more streamlined process for the everyday maintenance and tasks required to run a store.
...
While Blueprint uses Style Editor to customize legacy themes, Stencil uses Page Builder. [Page Builder](https://support.bigcommerce.com/s/article/Page-Builder) is BigCommerce's browser-based tool that enables retailers to rapidly modify and customize a storefront's look and feel without writing code. Stencil theme developers can configure settings for Page Builder. The configuration determines the theme's customizable aspects.

For example, by customizing the configuration of Page Builder, you can modify how retailers customize colors, fonts, display of page features, and the number of products displayed per feature.
...
Stencil allows theme developers and retailers to assign custom layout templates to the following types of storefront pages:
