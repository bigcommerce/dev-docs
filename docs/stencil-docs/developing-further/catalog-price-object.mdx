...
As a theme developer, you can use the catalog price object to highlight the savings that a retailer is offering over the MSRP directly on the storefront by referencing the product's `price` object and the correct property for the product.

Retailers can use control panel options to set the following prices for a catalog product:
...
| Cost | The product's `cost` property is never returned to the storefront by design. Generally, retailers would not want to reveal the true cost of goods to shoppers. The **Cost** field is meant for reports and third-party accounting integrations.|
...
These options and assigned values are each represented in the catalog price object, allowing theme developers to pull the information and render it onto the storefront to highlight the savings offered by a retailer.

## Catalog price object examples excluding tax

### No sale price defined in control panel

Assume that a retailer has defined the following for a product in the control panel:
...
Assume that a retailer has defined the following tax configurations for a product in the control panel:
...
Assume that a retailer has defined the following tax configurations for a product in the control panel, alongside a defined sale price:
