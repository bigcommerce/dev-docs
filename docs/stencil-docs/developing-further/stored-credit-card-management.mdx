...
{{/if}}
```
The `account_payments` variable includes the `<div>` element that renders the microapp and a script tag containing the microapp URL. You can use a script to attach the `renderAccountPayments()` method to the `window.BigCommerce` object. 

If the selected payment provider already has an integration with the Storefront account payments microapp, calling `renderAccountPayments()` renders the microapp automatically.
