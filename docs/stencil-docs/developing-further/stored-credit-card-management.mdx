...
After adding a stored payment method, shoppers can use the Storefront account payments microapp to manage their stored payment instruments. The Storefront account payments provide a payment method form with billing address fields. Currently, the microapp works only with Ayden.

### Microapp installation instructions

To install the Storefront account microapp to your theme, inject the `account_payments` variable into your template using `add-payment-method.html`.
...
{{/if}}
```
`account_payments` will provide the `<div>` element where you can render Storefront account payments and attach the renderAccountPayments() method to the window.BigCommerce object that displays the Storefront account payments app if the selected payment provider is already implemented.

A [Cornerstone example](https://github.com/bigcommerce/cornerstone/blob/master/templates/pages/account/add-payment-method.html#L45) shows how to load a script to the page. In this example, we have a condition: if `account_payments` is available - it renders; if not - the fallback payment method form renders.

In the subsequent releases, we'll remove the fallback payment method form because the Storefront account payments microapp will support all payment methods.

The following [Cornerstone example](https://github.com/bigcommerce/cornerstone/blob/master/assets/js/theme/account.js#L102) shows how to initialize the script. In `account.js` we need to call `window.BigCommerce.renderAccountPayments` to initialize the Storefront account payments app with the provided options:

The example has a simple initialize interface.
...
* `style` - an optional property for customization of the Storefront account payments form 
* `storeContextData` - a required field where you must provide data from the store context 
* `errorHandler` - a required field where you need to provide the callback that will be called on the Storefront account payments form submission error event.
