...
To install the Storefront account microapp to your theme, inject the `account_payments` variable into your template.
...
The following [Cornerstone example](https://github.com/bigcommerce/cornerstone/blob/master/assets/js/theme/account.js#L102) shows how to initialize the script. We need to call `window.BigCommerce.renderAccountPayments` to initialize the Storefront account payments app with the provided options:
