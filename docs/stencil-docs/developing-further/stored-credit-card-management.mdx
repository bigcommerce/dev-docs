...
A [Cornerstone example](https://github.com/bigcommerce/cornerstone/blob/master/templates/pages/account/add-payment-method.html#L45) shows how to load a script to the page. In this example, we have a condition because currently, the storefront-account-payments app works only with Adyen: if `account_payments` is available - it renders; if not - the fallback payment method form renders. If you install another payment provider, you still need to render the "add payments" form; therefore, the "add payments method" form displays.

Our roadmap includes plans for the Storefront accounts payment microapp to support all payment methods. Thus the fallback payment method is a temporary solution.

The following [Cornerstone example](https://github.com/bigcommerce/cornerstone/blob/master/assets/js/theme/account.js#L102) shows how to initialize the script. In `account.js` you need to call `window.BigCommerce.renderAccountPayments` to initialize the Storefront account payments app with the provided options:

The following is an example of a simple `initialize` interface.
