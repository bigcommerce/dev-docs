...
Early Hint reduces page load time and perceived latency by allowing browsers to download critical assets earlier in the request lifecycle. For more information, see [Early Hint](#early-hint).
...
```handlebars showLineNumbers
{{ earlyHint '{path}' '{REL}' [as='{AS}'] [crossorigin='{CORS}']}}
```

Retrieves the provided URL and produces an early hint to communicate important web page resources. The server indicates which site assets (such as CSS files, JS scripts, and fonts) a browser needs to render a page fully. When used in conjunction with the {{cdn}} helper, load times can improve by 30 percent.

#### Parameters

- `path` (String): The provided path used when determining web page resources.
- `REL` (String): Valid values are `preload`, `preconnect`, `prerender`,  and `dns-prefetch`.
- `AS` (String): Optional. Valid values are `script`, `style`, `font`.
- `CORS` (String): Optional. Valid values are `no` (default), `anonymous`, and `use-credentials`.


#### Example

```handlebars showLineNumbers
{{earlyHint 'samplepath.js' 'preload'}}
<!-- => 'https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js' 'preload' -->
```

In this example, the earlyHint helper produces the provided URL and an early hint. 

```handlebars showLineNumbers
{{ cdn 'assets/dist/theme-bundle.head_async.js' resourceHint='preload'}}
{{ earlyHint 'https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js' 'preload'}}
{{ getFontsCollection resourceHint='preload'}}
{{ stylesheet '/assets/css/theme/css' resourceHint='preload'}}
```

The output displays in a `Link` header.

```handlebars showLineNumbers
Link: <https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.8.3/angular.min.js>; rel=preload; type=application/javascript, <https://cdnjs.cloudflare.com/ajax/libs/vue/3.2.37/vue.cjs.min.js; rel=preload; type=application/javascript
Link: <https://bernardinainject-cloud-dev-vm.store.bcdev/store/sta8s37lcd/stencil/c1124950-05fa-013b-69a8-32020d8a3125/css/theme-ac056578-05fb-013b-2bf2-7a01a337b61f.css>; rel=preload; type=text/css, <https://fonts.googleapis.com/css?family Karla:400%7CMontserrat:400,500,700&display swap;rel=preload; <https://https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js>;rel=preload;type=application/javascript, <https://bernardinainject-cloud-dev-vm.store.bcdev/store/sta8s37lcd/stencil/c1124950-05fa-013b-69a8-32020d8a3125/dist/theme-bundle.head_asyn.js; rel=preload, <https://fonts.gstatic.com>; rel=dns-prefetch, <https://fonts.googleapis.com>; rel=dns-prefetch, <https://bernardinainject-cloud-dev-vm.store.bcdev/store/sta8s37lcd>; rel=dns-prefetch
```
