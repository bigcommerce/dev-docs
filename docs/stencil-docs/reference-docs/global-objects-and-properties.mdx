...
| `heading` | Heading message defined by the retailer. |
| `heading_color` | Color of the heading text. |
| `text` | Subheading defined by the retailer. |
| `text_color` | Color of the subheading text. |
| `button_text` | Text displayed on a call-to-action button defined by the retailer. |
...
**Description:** Defines elements of retailer’s store page
...
**Description:** Defines search parameters for the retailer’s site
...
| `address` | Store’s complete physical address, as entered by retailer in the [Store Profile](https://support.bigcommerce.com/s/article/Store-Profile-Settings).|
...
| `privacy_cookie` | If enabled, a string containing retailer-customizable text for (European Union–required) cookie-setting notification; if disabled, a boolean with a value of false. |
...
| `send_message` | URL to submit a message to the retailer `/account.php?action=send_message`. |
...
| `tax_label` | Defines the Tax Label (VAT, Sales Tax, etc.) that a retailer can set in an active MSF-enabled control panel’s **Settings > Tax** page for display to customers. |
...
| `message` | If store is in maintenance mode, string containing retailer-customizable body text (and optionally, HTML) for the "Down for Maintenance" window displayed to visitors; otherwise, null. |
| `notice` | If store is in maintenance mode, one of three strings identifying the store's status, and generating a corresponding text notice to retailers about how to proceed: AdminPrelaunchNotice for preview stores, AdminMaintenanceNotice for active stores taken down for maintenance, or AdminHibernationModeNotice for deactivated stores; otherwise, null. |
...
| `url` | URL to point to, set by retailer in control panel (https://www.facebook.com/BigCommerce, etc.). |
