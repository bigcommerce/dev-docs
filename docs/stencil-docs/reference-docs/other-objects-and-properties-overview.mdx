...
| out_of_stock_message | Retailer-defined label to display when a product is out of stock |
...
| stock_level | Current stock level of the product; will be null if storefront stock display is disabled by the retailer, or if the product lacks inventory tracking |
...
| availability | Optional availability message set by the retailer |
...
| warranty | Optional warranty text set by the retailer |
| meta_keywords | Optional search keywords that retailers may enter in the control panel’s "Add a Product" or “Edit a Product” page, to characterize the product in meta tags and storefront searches |
...
| &nbsp;&nbsp;description | Description (if entered by retailer) for this item |
| &nbsp;&nbsp;id | ID for this item |
...
| name | Retailer-defined category name |
| url | URL for the category-specific page |
| description | Retailer-defined description of the category |
...
| &nbsp;&nbsp;description | Retailer-defined description of the subcategory  |
...
| Property | Description |
|---|---|
| show_compare | Boolean corresponding to retailer’s control panel selection whether or not to enable product comparisons |
...
| sub_total | Price object that defines the total cost of all the items in the cart. Might or might not include tax, based on the tax display settings the retailer has configured. Excludes shipping, discounts, and gift wrapping. |
...
| &nbsp;&nbsp;show_estimator | Boolean indicating whether the retailer wants to show the shipping estimator to customers |
...
| &nbsp;&nbsp;availability | An optional availability message set by the retailer |
...
| returns_enabled | Boolean that indicates whether retailer allows products from the order to be returned |
...
| payment_instructions | Text field defined by retailer as to payment instructions for manual gateways such as “Bank Deposit” |
...
| reason | Reason for return; retailers can define actions beyond the default strings created with each store, which are: Received Wrong Product, Wrong Product Ordered, Not Satisfied With The Product, and There Was A Problem With The Product |
| action | Return action; retailers can define actions beyond the default set created with each store (Repair, Replacement, or Store Credit) |
...
| actions | Return actions: an array of strings arbitrarily defined by the retailer: refund, exchange, credit, etc. |
...
| &nbsp;&nbsp;public_key | Public key (site key) that reCAPTCHA returns upon retailer’s enabling CAPTCHA security in **Store Settings > Display Settings** |
...
**Description:****<em> </em>**Form object used to create and edit a gift certificate object for the retailer's store. Called on the default `<theme-name>templates/pages/gift-certificate/purchase.html` template. <br />
...
**Description:** Form object used to manage retailers’ "Contact Us" pages. Called on the default `<theme-name>/templates/pages/contact-us.html`template.
...
**Description:** Form object used to manage retailers’ "Login" page. Called on the default `<theme-name>/templates/pages/create-login.html`template.
