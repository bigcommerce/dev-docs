...
BigCommerce is currently targeting 11/1/2023 to sunset node-sass. To ensure that your storefront is up to date, use the latest node version (Node 18) in Stencil CLI and install the automated script to resolve [incompatible SCSS directives](/stencil-docs/installing-stencil-cli/troubleshooting-your-setup#incompatible-scss-directives), which can cause issues with the styling of your storefront.
</Callout>
