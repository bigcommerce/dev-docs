...
BigCommerce is currently targeting 11/1/2023 to sunset node-sass. To ensure that your storefront is up to date, use the latest version of node (Node 18) in Stencil CLI and install the automated script described in [Incompatible SCSS directives](#incompatible-scss-directives) to prevent an issue with the styling of your storefront.
</Callout>
