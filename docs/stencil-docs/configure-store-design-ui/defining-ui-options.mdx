...
To provide retailers with Store Design customization options, you must declare those options in the theme's <span class="fn">schema.json file</span>. You must also include those settings in your theme's <span class="fn">config.json</span> file, templates, and Sass/CSS files. The basic division of labor is this:
...
* For any theme setting (such as a Sass variable or a front-matter value) to be retailer-customizable,
...
* Retailers must select one variation to edit, at a time, in Store Design. The selections that they make in the remainder of Store Design's UI will apply to only that selected variation.
...
In all, the structure that you give your theme's <span class="fn">config.json</span> and <span class="fn">schema.json</span> files directly governs the UI that Store Design exposes to retailers. So these files provide your UI design tools.
...
```json filename="Example object data type declaration schema.json" showLineNumbers
 {
        "type": "color",
        "label": "Text Color",
        "id": "body-font-color"
...
```json filename="'Heading' type settings schema.json" showLineNumbers highlightLines=[[5]]
{
    "name": "Colors",
    "settings": [
      {
...
These `"type": "heading"` statements do not reference data types. Rather, they declare display captions for the Store Design UI's subcategories – the middle level nested within the Section headings, but outside the individual options from which retailers can select. (Those inner options are designated by `"label": <label-text>` statements.)

## Store Design Data Structure in <span class="fn">schema.json</span> 

The <span class="fn">schema.json</span> nesting structure that you just saw maps directly to the Store Design UI displayed to retailers: Below the `variations` section (whose data are imported from <span class="fn">config.json</span>), the order and nesting of options in Store Design's UI directly matches the order and nesting of your <span class="fn">schema.json</span> entries.
