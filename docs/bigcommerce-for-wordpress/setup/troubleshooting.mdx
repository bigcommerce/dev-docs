...
When using the `Fast` (headless) import, be sure to apply the right scopes to your API keys. The Storefront API tokens, and Storefront API impersonation token, scopes are required. Create a new token with the correct scopes by going to **Dashboard > Settings > Store-level API accounts**.

## Mini-cart/cart page doesn't render or the wrong amount of products appear

Check to see if this is a cache problem. Each time a product is added to a cart, an API request is sent. Without a cache set, the cart's content is updated after the successful processing of the API request. However, if cache rules are applied they can affect the cart and return the wrong version of the cart (e.g. empty cart page, not a sufficient amount of products, etc.). 
...
If missing products in the WordPress control panel, verify that products are assigned to the corresponding channels. Make sure that the `Automatic Listing` option for `Full` import isn't disabled in the plugin's settings.
...
If you see the following webhook error, shown below, try resetting the webhooks in the BigCommerce control panel.
...
The message in the error logs mean that a product has already been imported to WordPress and it won't be processed again
...
If products are no longer updating after an import, then flush the cache on your WordPress setup. In some cases pages and product information is cached and old information is shown.
