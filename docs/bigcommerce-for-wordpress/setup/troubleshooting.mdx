...
Reading an error log is the first step in diagnosing common issues and error messages. To effectively troubleshoot problems the error logs and diagnostics setting must be enabled for the plugin. From the WordPress admin panel select **BigCommerce** > **Settings** > **Diagnostics** and click **Enable Error Logs and Get Diagnostics**. Now you're set to determine which topic listed here can help you get back up and running.
...
If products are no longer syncing, or progress has stalled, check to see if WP-Cron is working. Determine if your WP-Cron job is running by using the plugin WP Crontorl. 

Make sure that you have a browser window open on the site, otherwise the product import will continue to fail. If WP-Cron isn't working, then reach out to the site's hosting provider.
...
If you get a 404 error, or are confronting an issue with the URL, you can fix this in the WordPress control panel. Be sure to set the correct permalink structure in the WordPress admin panel by clicking **Settings** > **Permalinks**. For correct plugin work, enable any permalink structure except a plain one.
...
If you repeatedly encounter the same error or warning and this page doesn't solve your issues then please create an Issue in our [GitHub repository](https://github.com/bigcommerce/bigcommerce-for-wordpress/issues). Be sure to provide as many details as possible including the BigCommerce for WordPress plugin version, PHP version, and my SQL version.

<Callout type="info">
  Sometimes errors don't appear in the BigCommerce for WordPress plugin logs but they can still affect the plugin. If you have exhasted all triaging steps then it's suggested that you enable server error logs. You can usually find these settings in your server's control panel or admin dashboard.
</Callout>
