...
When in use the plugin floods the error log with the following message:
...
When sending an API request you may come across a 403 error, or you might encounter the following message:
...
Make sure that either the server, or service, isn't blocking requests. Be sure to whitelist requests to BigCommerce API and the responses from it.

## Could not retrieve the token

There could be API scope issues if you get the error below:
...
When using the `Fast` (headless) import, be sure to apply the right scopes to your API keys. The Storefront API tokens and Storefront API impersonation token scopes are required. Create the correct scopes by going to **Dashboard > Settings > Store-level API accounts**.

## Mini-cart/cart page doesn't render or the wrong amount of products appear

Check to see if this is a cache problem. Each time a product is added to a cart, an API request is sent. Without a cache set, the cart's content will be updated after the successful processing of the API request. However, if cache rules are applied they can affect the cart and return the wrong version of the cart (e.g. empty cart page, not a sufficient amount of products, etc.). 
...
3. Now try to restart the import. Check to see if WP-Cron is working and that there aren't any server issues. 

<Callout type="info">
  If import still doesn't work, contact BigCommerce support and/or create an issue in the [GitHub repository](https://github.com/bigcommerce/bigcommerce-for-wordpress/issues). 
...
If you come across missing products in the WordPress control panel, verify that products are assigned to the corresponding channels. Make sure that the `Automatic Listing` option for `Full` import isn't disabled in the plugin's settings.

## Import is too slow

The import time is dependent on the amount of products that you have synced from your catalog. In general, if you have more than 200 products and use `Full` import,  then it is normal for the import to run for more than 30 minutes. 
...
If you find that the product import is canceled, or see the following error below, make sure your channel is properly connected.
...
Check that your WordPress store is connected to BigCommerce, and that the primary channel is set under **BigCommerce > Settings > Channel**. If the primary channel is not set, then re-install the plugin and connect it to the correct store channel.

## Webhooks incorrect password

If you see the following webhook error, shown below, try resetting webhooks in the BigCommerce control panel.
...
If you get a 404 error, or are confronting an issue with the URL, you can fix this in the WordPress control panel. Be sure that the correct permalink structure is set in **wp-admin > Settings > Permalinks**. For correct plugin work enable any permalink structure except a plain one.
...
The message in the error logs mean that a product has already been imported to WordPress and will not be processed again

## Failed to download image. cURL error 28: Resolving timed out after 10000 milliseconds

If the images are failing a download, or you come across the following error below, there are steps to resolve this.
...
* DNS failure
* Server limits (e.g some proxy set on server and it has its own timeout limit)
* Security modules (Mod_Security) may block requests
* Server is trying to resolve bigcommerce.com domain using IPv6 instead of IPv4

## E-commerce pages(categories, products single page, checkout) are not visible for non-logged user

Make sure that your channel that's attached to WordPress isn't in pre launch status. If its in prelaunch status, change it to active and perform an import on the WordPress side.

## New/Updated since last sync doesn't work on Fast import

The `Fast Headless` import doesn't support the "New/Updated since last sync" (partial) option. The fast import option only retrieves categories, brands, and product IDs; all other information will load on the corresponding front page.

## What scopes are necessary for full functionality

The full list of scopes can be found on the [Multi-site Setup](https://developer.bigcommerce.com/bigcommerce-for-wordpress/setup/multisite#api-account-name-field) page.

## Products are not updated after import

If products are no longer updating after an import, then flush the cache on your WordPress setup. In some cases pages and product information may be cached and old information is shown.
...
1. Go to the **Appearance > Customizer > BigCommerce > Product Single > Pricing nonce field(for advanced use only)** and disable nonce.
