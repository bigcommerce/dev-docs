...
The shopping frontend of B2B Edition for Stencil runs on Stencil storefronts alone, whereas the [B2B Edition Buyer Portal]() is storefront technology-agnostic. The Buyer Portal uses the [Scripts API](/api-docs/store-management/scripts-overview) to deploy a React app that accomplishes many of the same things as B2B Edition for Stencil. For more on the differences, see our Help Center article on the [Buyer Portal](). 

## Customizing B2B Edition for Stencil

With B2B Edition for Stencil, you can customize display text, styling, checkout configurations, and the placement of app elements. You also have access to the lifecycle methods for many B2B Edition objects and can inject your own JavaScript functions.
...
This section gives details on how to edit the DOM, CSS, and display text in a B2B Edition Stencil theme, and how to define JavaScript lifecycle methods. It assumes you are familiar with editing Stencil themes locally and pushing them to a sandbox or production store using the Stencil CLI.
