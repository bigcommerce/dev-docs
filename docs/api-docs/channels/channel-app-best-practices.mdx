...
As the specific needs of every retailer is different, the features of your app should be as modular as possible.

### For retailers

Consider the audience and various use cases for your application:

- **Retailer A**: only interested in importing products and inventory.
- **Retailer B**: interested in a bi-directional integration of products and sales that keeps data between the channel and BigCommerce in sync.

Retailers should have the option to opt-in or out of the various features of your application.

### For support

If an issue occurs, a retailer should have the option to disable a specific feature that may not be behaving as expected. The idea here is to allow the retailer to continue using the features that are functional while temporarily resolving an issue - instead of having to disable the application completely.
...
This will provide the retailer with more control over what is synced and when.

Example Automated Sync Intervals:

- **Do not update automatically**: The retailer should have an option to opt-out of automatic syncs.
...
To provide retailers with a positive integration experience, we expect point of sale applications to meet or exceed the following benchmark:
...
A log of all events should be kept and made accessible to the retailer utilizing the application.
...
The retailer should also have the option to download verbose logs in CSV format:
