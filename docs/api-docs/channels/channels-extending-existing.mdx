...
See our [Guide to API Accounts](/api-docs/getting-started/api-accounts) for more information on app credentials.
...
- [Create a Channel](/docs/rest-management/channels) (for each platform the app enables a retailer to sell on) - this enables the channel to be displayed within the “Manage” screen in the new Channel Manager for retailers, once the app has been installed.
...
- **Channels** - Register a channel for all existing retailers using the app.
- **Orders** - Add `channel_id` with the corresponding channel ID for the retailer and which channel the order was placed on, if orders are synced to BigCommerce.
...
[{
    ...
    "name": "name of the sales channel",  // Name displayed to retailer
    "external_id": "",
    "status": "connected",
    "config_meta": {
      "app": {
        "id": 123,                        // ID of the app
        "sections": [{                    // Label displayed to retailer in BigCommerce navigation bar
...
| `name` | str | The name the retailer will see |
...
| &nbsp;&nbsp; ↳ `title` | str | Label displayed to retailers in navigation bar |
...
| **Select Partners**  | Partners approved by BigCommerce to be marketed and discoverable as sales channel providers within the new Channel Manager. These are partners who offer what we know to be best in breed integrations to key sales channels that help retailers grow their business.|
