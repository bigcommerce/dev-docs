...
The Payments API enables you to process payments through the store’s connected payment gateway. Merchants can receive a payment for an order that was created using either the [Server to Server Checkout API Orders](/docs/rest-storefront/checkouts) endpoint or the [V2 Orders](/docs/rest-management/orders#create-an-order) endpoint.
...
Consult the [Payments API Reference](/api-docs/store-management/payments-api-overview) to determine which OAuth permission scopes your application's [API Account](/api-docs/getting-started/api-accounts) needs.
Learn more about BigCommerce API [OAuth scopes](/api-docs/getting-started/api-accounts#oauth-scopes).
</Callout>
...
1. Make a call to [Get Payment Methods](/docs/rest-payments/processing/accepted-methods#get-accepted-payment-methods) for the `stored_instruments > token` to pay with a stored card. The `order_id` passes in as a query parameter.
...
2. Make a request to [Create Access Token](/docs/rest-payments/processing/access-tokens#create-payment-access-token) to get the authorization token that needs to be passed in the header when processing the payment. The ID of the order needs to be part of the request body.
...
3. To process the payment, send a POST request to [Process Payment](/docs/rest-payments/processing/process-payment#process-payment). You will need several values retrieved with the [Get Payment Methods](/docs/rest-payments/processing/accepted-methods#get-accepted-payment-methods) request you made in a preceding step. Additionally, this request contains different headers than a typical BigCommerce API request.  Consult the following for more information:
...
1. Make a request to [Create Access Token](/docs/rest-payments/processing/access-tokens#create-payment-access-token) to get the authorization token that needs to be passed in the header when processing the payment. The ID of the order needs to be part of the request body.
...
2. To process the payment, send a POST request to [Process Payment](/docs/rest-payments/processing/process-payment#process-payment). You will need several values from the customer's credit card. All the example request body values are required.  If any of these values are incorrect, you may be unable to process the payment. Additionally, this request contains different headers than a typical BigCommerce API request.  Consult the following for more information:
...
| `30051` | That stored payment instrument could not be found. Please try a different payment option. |  The card requested for payment is not associated to the shopper.| Use [Get Payment Methods](/docs/rest-payments/processing/accepted-methods#get-accepted-payment-methods) to see available vaulted cards |
...
### Endpoints
* [Create Access Token](/docs/rest-payments/processing/access-tokens#create-payment-access-token)
* [Get Payment Methods](/docs/rest-payments/processing/accepted-methods#get-accepted-payment-methods)
* [Process Payment](/docs/rest-payments/processing/process-payment#process-payment)
