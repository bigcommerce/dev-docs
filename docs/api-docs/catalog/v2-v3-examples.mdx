...
When a product option is created in V2 and assigned to a product, editing the global option using the V3 Catalog API will automatically copy the V2 global product option to a local product variant, option, or modifier. This is triggered by an `UPDATE` or a `DELETE` call to either the [Product Options](/docs/rest-management/catalog/variants#get-all-variants) or [Product Modifiers](/docs/rest-management/catalog/product-modifiers#get-all-product-modifiers) endpoints.
