...
The following example creates a base product, variant options, and variants in a single call to the [Products](/docs/rest-management/catalog/products#create-a-product) endpoint. Use this method to create a product and variants in a single call without creating variant options first (option display will default to radio button).
...
To [create a modifier](/docs/rest-management/catalog/product-modifiers#create-a-product-modifier), send a `POST` request to `/v3/catalog/products/{{product_id}}/modifiers`.
...
To [update the modifier value](/docs/rest-management/catalog/product-modifier-values#update-a-modifier-value), send a `PUT` request to `/v3/catalog/products/{{product_id}}/modifiers/{{modifier_id}}/values/{value_id}`.
...
To create a [Brand](/docs/rest-management/catalog/brands#get-all-brands), send a `POST` request to `/v3/catalog/brands`.
...
[Category Tree](/docs/rest-management/catalog/category-trees#get-a-category-tree) returns a simple view of the parent > child relationship of all categories in the store. You can use this endpoint to fetch the categories if building out a custom navigation for a store.
...
[Product Sort Order](/docs/rest-management/catalog/product-sort-order) allows you to manage the sort order of products displayed on any given category page. Products assigned to multiple storefront categories can have different sort order values per category.
