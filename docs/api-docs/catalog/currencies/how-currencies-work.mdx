...
Catalog search and filtering by price only work for the default currency and auto-converted pricing for non-default transactional currencies. If a retailer sets up pricing through **Price Lists** and has price filter enabled on their store, shoppers will be unable to see any products when searching by price.

* `Shop by: Price` only works for default currency and auto-converted pricing for non-default transactional currencies. If retailer sets up pricing through price lists and has price filter enabled on their store, shoppers will be unable to see any products when searching by price.

BigCommerce does not dynamically convert currency. To convert, the retailer will need to do one of the following:
...
* Set up explicit pricing per each currency using price lists (only available to Enterprise retailers).
