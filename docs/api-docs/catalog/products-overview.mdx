...
* **Physical** - Exist in a physical form, have a weight, and are sold by retailers to ship to customers.
...
The following example creates a base product, variant options, and variants in a single call to the [Products](/docs/rest-catalog/products#create-a-product) endpoint. Use this method to create a product and variants in a single call without creating variant options first (option display will default to radio button).
...
[Modifier options](/docs/rest-catalog/product-modifiers#get-all-product-modifiers) are any choices that the shopper can make to change how the retailer fulfills the product. Examples include:
...
Critically, the modifier will not change the SKU/variant fulfilled, and you cannot track inventory against combinations of modifier values. Modifiers typically would not change which product is “picked off the shelf” in the warehouse, but they change what happens to that product before sending it to the shopper, or how a retailer can send it.
...
[Complex rules](/docs/rest-catalog/products/complex-rules#get-complex-rules) allow retailers to set up conditions and actions based on shopper option selections on the storefront. You can use them to vary the following based on the shopper's option selections:
* Price
* Weight
* Image
* Purchasability
...
For most retailer use cases, **best practice** will be to either assign values (such as a price) directly to a variant or use adjusters on the modifier option itself. However, complex rules exist for rare cases where a rule condition is too complex to express in those forms easily.
