...
3. After setting up the channel, you can authenticate cross-origin requests by [creating a JWT token](/docs/storefront-auth/tokens#create-a-token) through the GraphQL Storefront API. Make sure to create your GraphQL Storefront API token with the same channel ID as your headless platform; otherwise, your request will be rejected.

<Callout type="info">
#### Limits
When the BigCommerce storefront status is `inactive` or `prelaunch`, one active headless storefront and a free store are allowed.
</Callout>
