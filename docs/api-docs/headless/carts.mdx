...
To generate a cart redirect URL, send a request to the [Create cart redirect URL](/api-reference/store-management/carts/cart-redirect-urls/createcartredirecturl) endpoint. Use the `id` returned with the [Create a cart](/docs/rest-management/carts/carts-single#create-a-cart) response for the `cartId` path parameter.
