...
| `channel_id` | string | Channel ID of the channel where checkout is taking place. Not needed for GA4. | [Get a checkout](/api-reference/store-management/checkouts/checkout/checkoutsbycheckoutidget) <br> `cart_amount_inc_tax` | Not sent to Google Analytics. |
...
| `line_items[]` | array | Items being purchased. | See [Line items](#common-event-fields-line-items) | [Purchase event](https://developers.google.com/analytics/devguides/collection/ga4/reference/events#purchase) <br> `purchase.items[]` |
