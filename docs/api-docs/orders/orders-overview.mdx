...
Specify the [customer](/docs/rest-management/customers#get-all-customers) by including a `customer_id` in the [create order](/docs/rest-management/orders#create-an-order) request.
...
To get the `product_options.id` and `product_options.value` of a product for the order `products` array, send the following `GET` request to [Get variants by product id](/docs/rest-management/catalog/product-variants#get-a-product-variant). See the example response that follows, or consult the [response schema](/docs/rest-management/catalog/product-variants#get-a-product-variant).
<Tabs items={['Request', 'Response']}>
<Tab>
...
To [get shipping quotes](/docs/rest-management/orders/order-shipping-addresses-quotes#get-order-shipping-quotes), send the following `GET` request. See the example response that follows, or consult the [response schema](/docs/rest-management/orders/order-shipping-addresses-quotes#get-order-shipping-quotes). 
<Tabs items={['Request', 'Response']}>
<Tab>
...
[Orders V3](/docs/rest-management/transactions) exposes endpoints for managing [order refunds](/docs/rest-management/transactions/payment-actions#get-refunds-for-order). For an overview on using these endpoints, see [Order Refunds in API Docs](/api-docs/orders/payment-actions).
