...
| Links displayed in a control panel | Optional | URL, Title, Description | Link(s) displayed to merchants when they navigate to **Settings > Setup > Tax > _TaxProviderName_**. One or multiple links supported. | `support.sampletax.com`, `Sample Tax Support`, `The Sample Tax Support website.` |
**Coverage** | | | | |
...
We recommend calling the [Update Connection](/docs/apps-api/tax-app-connection#update-a-connection) endpoint immediately after the app has been successfully installed, otherwise your tax provider will not be displayed when merchants navigate to the **Settings > Setup > Tax** page in a BigCommerce control panel.
...
To enable the tax provider, merchants must navigate to **Settings > Setup > Tax** in a BigCommerce control panel and click **Enable** next to the associated tax provider.

If document submission is supported, navigate to the provider's settings menu, found at **Settings > Setup > Tax > _TaxProviderName_** in a BigCommerce control panel and ensure the **Submit Order Data** checkbox is checked.
...
* Test connection functionality when merchants navigate to **Settings > Setup > Tax > _TaxProviderName_** in a BigCommerce control panel
...
When responding to tax estimate requests sent by BigCommerce, tax providers are required to include aggregates and breakdowns of sales tax amounts and rates for product item prices, shipping and handling prices. This is because when navigating to the **Settings > Setup > Tax > Tax Settings** page in a BigCommerce control panel, merchants have the ability to specify whether they would like to show taxes in cart, checkout, orders and invoice **As one summarized line item** or **Broken down by tax rate**.
...
If document submission is supported, navigate to **Settings > Setup > Tax > _TaxProviderName_** in a control panel and ensure the **Submit Order Data** checkbox is checked.
...
To test the tax provider connection, navigate to **Settings > Setup > Tax > _TaxProviderName_** in a BigCommerce control panel and click **Test Connection**. This selection will trigger a sample tax estimate request to be sent to the estimate URL provided by the tax provider. If the connection is unsuccessful, merchants can navigate to **Settings > Advanced > Store logs** in a BigCommerce control panel to view the error and its description to assist with triaging the issue.
