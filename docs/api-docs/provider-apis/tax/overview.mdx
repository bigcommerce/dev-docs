...
Once you've obtained your app ID, share your app ID as well as the below information with BigCommerce by emailing [taxproviderapi@bigcommerce.com](mailto:taxproviderapi@bigcommerce.com). This allows us to create your tax provider configuration, which will take approximately five days.
...
| Links displayed in an active MSF-enabled control panel | Optional | URL, Title, Description | Link(s) displayed to merchants when they navigate to **Settings > Setup > Tax > _TaxProviderName_**. One or multiple links supported. | `support.sampletax.com`, `Sample Tax Support`, `The Sample Tax Support website.` |
**Coverage** | | | | |
...
Once the tax provider's app is successfully installed and basic authentication credentials are provided through the Update a Connection request, merchants can enable the tax provider on all test stores provided by the tax provider in [sharing provider details with BigCommerce](#sharing-provider-details-with-bigcommerce).
...
If document submission is supported, navigate to the provider's settings menu, found at **Settings > Setup > Tax > _TaxProviderName_** in an active MSF-enabled BigCommerce control panel and ensure the **Submit Order Data** checkbox is checked.
...
* Test connection functionality when merchants navigate to **Settings > Setup > Tax > _TaxProviderName_** in an active MSF-enabled BigCommerce control panel
...
If document submission is supported, navigate to **Settings > Setup > Tax > _TaxProviderName_** in an active MSF-enabled control panel and ensure the **Submit Order Data** checkbox is checked.
