...
The [Tax Provider API](/docs/apps-api/tax) allows you to provide business-to-consumer sales tax estimates to shoppers on the storefront and to retailers in the control panel; it can also be used to submit documents for tax reconciliation purposes.
...
Multi-tenant tax providers can publish their BigCommerce app so that it's discoverable by anyone or publish their app as unlisted so it can only be installed using a URL. Furthermore, tax solutions that have been built in-house or for specific retailers by digital agencies are also supported as private instances and will only work on the specified stores.
...
| Partner support email | Required | Email | Used by BigCommerce to contact tax provider, in the case we need to forward a retailer support request. | `support@sampletax.example.com` |
| Links displayed in the control panel | Optional | URL, Title, Description | Link(s) displayed to retailers when they navigate to **Settings > Setup > Tax > _TaxProviderName_**. One or multiple links supported. | `support.sampletax.com`, `Sample Tax Support`, `The Sample Tax Support website.` |
**Coverage** | | | | |
| Tax provider visibility | Required | Show, Hide | Tells us if tax provider should be displayed to retailers in the control panel before the tax providers app is installed on the associated store. | `Show` |
...
<sup>+</sup> You have the option of supporting a URL that retailers can customize. See [Tax Profile](#tax-profile-optional).
...
If you would like to support multiple connections for a retailer, BigCommerce can set the registry with flexible URLs that use our new profile option. 
These URLs would include a reserved string, for example, `https://{profile}/estimate`, allowing retailers to customize the base domain.   

When connecting or installing your app, you must allow retailers to enter the URL's base domain in the control panel. Retailers need to be able to self-serve updates to the URL through your app. 
When you [Establish a Tax Provider connection](#establishing-a-connection), include the `profile` base domain along with credentials associated with the retailer's account. 

<Callout type="info">
  You must liaise with the retailer to provide them with customized URLs. BigCommerce merely consumes the URL values the retailer enters and uses the provided string value for Tax Provider API requests.
</Callout>
...
When the retailer clicks **Install** on a tax provider's single-click app, it's essential that the app successfully handle the OAuth flow before moving on to the next step of establishing a connection with the [Tax Provider API](/docs/apps-api/tax). The [Apps Guide](/api-docs/apps/guide/auth) provides an overview of the OAuth flow.
...
* Tax provider has shared their tax provider details with BigCommerce through email, and we've replied confirming the tax provider configuration is now ready on all supplied retailer and/or partner test stores (see [Sharing provider details with BigCommerce](#sharing-provider-details-with-bigcommerce)).
...
* During the OAuth flow the app requested the retailer to authorize read and write permissions on the Information and Settings API scopes (see [Installing the app](#installing-the-app)).
...
If the tax provider supports all eligible stores, then they may choose to provide an account registration flow in their app iFrame in order to capture these basic authentication credentials from retailers. Learn more about designing the app UI [here](/api-docs/apps/guide/ui).
...
We recommend calling the [Update Connection](/docs/apps-api/tax-app-connection#update-a-connection) endpoint immediately after the app has been successfully installed, otherwise your tax provider will not be displayed when retailers navigate to the **Settings > Setup > Tax** page in the control panel.
...
Once you have successfully installed the tax provider's app and provided basic authentication credentials through the Update a Connection request, retailers can enable the tax provider on all test stores supplied by the tax provider in [sharing provider details with BigCommerce](#sharing-provider-details-with-bigcommerce).

To enable the tax provider, retailers must navigate to **Settings > Setup > Tax** in the control panel and click **Enable** next to the associated tax provider.
...
* Test connection functionality when retailers navigate to **Settings > Setup > Tax > _TaxProviderName_** in the control panel
...
When responding to tax estimate requests sent by BigCommerce, tax providers are required to include aggregates and breakdowns of sales tax amounts and rates for product item prices, shipping and handling prices. This is because when navigating to the **Settings > Setup > Tax > Tax Settings** page in the control panel, retailers have the ability to specify whether they would like to show taxes in cart, checkout, orders and invoice **As one summarized line item** or **Broken down by tax rate**.
...
Prior to testing a tax provider, the retailer or partner test store should have the following configured in the control panel:
...
To test the tax provider connection, navigate to **Settings > Setup > Tax > _TaxProviderName_** in the control panel and click **Test Connection**. This selection will trigger a sample tax estimate request to be sent to the estimate URL provided by the tax provider. If the connection is unsuccessful, retailers can navigate to **Settings > Advanced > Store logs** in the control panel to view the error and its description to assist with triaging the issue.
