...
BigCommerce's Store Management and Payment APIs (for example, the [Catalog API](/docs/rest-management/catalog)) allow you to manage transactions, modify store data, and act as store administrator. Example use cases include the following:
...
BigCommerce's [GraphQL Storefront API](/docs/graphql-storefront) allows you to query product, customer, and store settings headlessly or from a store's frontend. Example use cases include the following:
...
| [Enabled Payment Methods](/docs/rest-payments/methods#get-all-payment-methods) | Get a list of payment methods available on a store. |
...
```json filename="Example request body: Product object" showLineNumbers
{
 "id": 5,
 "name": "iPod",
...
```json filename="Example response: Get a category" showLineNumbers
{
  "data": {
    "id": 39,
    "parent_id": 19,
