...
| | "GraphQL invalid credentials. JWT signature is invalid" | One of the JWT claims fails (token was not generated by BC) |
| | "GraphQL invalid credentials. JWT is expired" | Token is expired |
...
| | "GraphQL invalid credentials. JWT store id doesn't match store id of the URL" | Store ID mismatch (store ID in the token does not match store ID of the site/URL) |
