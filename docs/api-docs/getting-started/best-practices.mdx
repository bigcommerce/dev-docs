...
To keep data in your application up-to-date, [webhooks](/api-docs/getting-started/webhooks/about-webhooks) provide a great alternative to periodic API polling. Use an [OAuth API account](/api-docs/getting-started/api-accounts) to register and subscribe to webhook-enabled events that are relevant to your application.
...
```http filename="Example: 429 response"
...
```php filename="PHP example for delaying response"
