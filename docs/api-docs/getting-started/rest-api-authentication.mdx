...
Every active API account has at least one `access_token`. [Store-level API accounts](#store-level-api-accounts) include a static `access_token` that does not change. [App-level API accounts](#app-level-api-accounts) expect each application to generate a unique `access_token` for every store that installs the app.[Account-level](#account-level-api-accounts) creates a token for a single account; the token is tied to the account that the user is signed into.

**Guard these values closely.** The `client_id` and `client_secret` will never change; `access_token`s do not expire based on time and cannot be manually invalidated. It's best practice to limit each account's [OAuth scope](#oauth-scopes) to only the privileges needed to complete that app or user's designated tasks. Create separate API accounts for each app, store API user, account API credentials, and/or function.

## Store-level API accounts

Merchants generate single-store API credentials when they create API accounts in their store control panel (**Advanced Settings** > **API Accounts**). Use these credentials to read and change one store's data with BigCommerce's APIs. You can't change store-level API accounts' access tokens or OAuth scopes. 
...
### Obtaining Store-level API credentials

To create a store-level API account, consult our Knowledge Base article on [Creating a Store API Account](https://support.bigcommerce.com/s/article/Store-API-Accounts?language=en_US#creating).
...
### Obtaining app-level API credentials
...
### Obtaining account-level credentials 

To use the account-level API, merchants must generate an API token from the BigCommerce control panel (**Settings** > **Account-level API Tokens**). Learn more about setting up Account-Level API token by checking out our Knowledge Base article, [API Accounts](https://support.bigcommerce.com/s/article/Store-API-Accounts?language=en_US#Account-Level-api-accounts).

<Callout type="info">
    When a token is generated, it will belong to the account that the merchant has logged into.
</Callout> 

The API token should be kept secure, as it provides complete access to the account's stores. This means that any actions performed through the API have the same effect as if they were performed directly in the store's control panel. 

### Revoking account-level API credentials
An account-level API token is valid until the account is no longer active. However, account-level credentials can be manually deleted. From the account's control panel, click **action** to access the delete option.

<Callout type="warning">
 Do be aware that deleting APIs must be done with precaution. Only users who have the correct permission can perform this action.
</Callout>

Learn more about deleting an account-level API account by visiting our Knowledge Base article [API Accounts](https://support.bigcommerce.com/s/article/Store-API-Accounts?language=en_US#deleting)
...
Where all three types of API accounts are supported, review the preceding sections to make an informed choice about which best fits your use case. In the following table, links go to the relevant section of our [Authentication and Example Requests](/api-docs/getting-started/authentication) article.

| API or Use Case | App-level API Account | Store-level API Account | Account-level API Account |
|:----------------|:---------------:|:-----------------:|:---:|
| [REST Store Management APIs](/api-docs/getting-started/authentication#access-tokens) | &times; | &times; | |
| [GraphQL Admin API](/api-docs/getting-started/authentication#access-tokens) | &times; | &times; | &times; |
...
| [Customer Login API](/api-docs/getting-started/authentication#user-generated-jwts) | &times; |  | &times; |
| [Current Customer API](/api-docs/getting-started/authentication#client-id) | &times; |  | &times; |
| [Payments API](/api-docs/getting-started/authentication#bigcommerce-generated-jwts) | &times; | &times; |
| [Apps that host REST Provider APIs (provider apps)](/api-docs/getting-started/authentication#developer-configured-authentication) | &times; |  | &times; |
| Apps hosted in the store control panel (single-click app) | &times; |  | &times; |
...
|:--------|:-----------|:----------|:------------|:----------|
| Create Payments | modify | `store_payments_access_token_create` | Create payment access tokens, process payments | [Payments, Access Tokens](/docs/rest-payments/processing/access-tokens#create-payment-access-token) <br /> [Payments, Process Payment](/docs/rest-payments/processing/process-payment#process-payment) |
...
| Get Payment Methods | read-only | `store_payments_methods_read` | Get a list of payment methods | [Payments, Methods](/docs/rest-payments/processing/accepted-methods#get-accepted-payment-methods) |
| Products | modify | `store_v2_products` | View and modify products, brands, categories, and other product information. | [Catalog V3](/docs/rest-management/catalog) <br />[Price Lists V3](/docs/rest-management/price-lists) |
| Products | read-only | `store_v2_products_read_only` | View products | [Catalog V3](/docs/rest-management/catalog) <br />[Price Lists V3](/docs/rest-management/price-lists) |
...
| Account | read-only | `account_read` | View account details | [GraphQL Users API Overview](/docs/graphql-users/overview) |
| Account Stores | read-only | `account_stores_read` | View the stores associated with an account | [GraphQL Users API Overview](/docs/graphql-users/overview) |
| Account Apps | read-only | `account_apps_read` | View the apps associated with an account | [GraphQL Users API Overview](/docs/graphql-users/overview) |
| Account Users | read-only | `account_users_read` | View the users associated with an account | [GraphQL Users API Overview](/docs/graphql-users/overview) |
| Account Users | write | `account_users_write` | Add a user to an account | [GraphQL Users API Overview](/docs/graphql-users/overview) |
| Account Users | write | `account_users_write` | Create a user with a password for an account | [GraphQL Users API Overview](/docs/graphql-users/overview) |
| Account Users | delete | `account_users_delete` | Remove a user from an account | [GraphQL Users API Overview](/docs/graphql-users/overview) |
