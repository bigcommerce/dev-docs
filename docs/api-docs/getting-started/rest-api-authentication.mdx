...
Merchants generate single-store API credentials when they create API accounts in their store control panel (**Settings** > **API** > **Store API Tokens**). Use these credentials to read and change one store's data with BigCommerce's APIs. You can't change store API accounts' access tokens or OAuth scopes. 
...
Merchants can generate account-level API credentials when they create API accounts in their store control panel (**Settings** > **API** > **Account API Tokens**). 
