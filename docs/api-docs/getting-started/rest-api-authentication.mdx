...
[Store-Level API accounts](#store-api-accounts) include a static `access_token` that does not change 
[Account-level API accounts](>>>>URL<<<<) grants merchants the ability to create an `access_token` that is tied to an account.

**Guard these values closely.** The `client_id` and `client_secret` will never change; `access_token`s do not expire based on time and cannot be manually invalidated. It's best practice to limit each account's [OAuth scope](#oauth-scopes) to only the privileges needed to complete that app or user's designated tasks. Create separate API accounts for each store API user, or for each account that belongs to a merchant.

## Store-level API accounts

Merchants generate store-level API credentials when they create API accounts in their store's control panel (**Settings** > **API** > **Account-level API Tokens**). Use these credentials to read and change the single-store data with BigCommerce's APIs. You can't change store-level API accounts' access tokens or OAuth scopes. 
...
To revoke store-level API credentials, you must delete the corresponding store-level API account. If the `client_id` and `client_secret` are compromised, or the account has become unnecessary, secure your account by deleting the API account. You cannot recover a deleted API account, so take precaution.

<Callout type="error">
  #### Delete carefully
  Deleting an account can't be undone, so be sure before clicking the trash can icon. You can also use the checkboxes on the left side to delete multiple accounts at once – but be especially careful when using this option.
</Callout>
...
Merchants can generate account-level API credentials when they create an account API in their store's control panel (**Settings** > **API** > **Account API Tokens**). This type of API account grants merchants the ability to manage all stores tied to the specific account and not the user that created it. With account-level credentials, merchants can secure the OAuth scopes necessary to manage users on a larger scale.

Since this API token is tied to a specific account, the token remains active unless the Merchant's account becomes inactive. The account-level API allows merchants to make API calls to our platform, with the help of our API Gateway, allowing merchants to provide an automated onboarding or offboarding experience on an enterprise level. There are other ways that BigCommerce's account-level API can assist with handling all provisioned users. For example, merchants can leverage an account-level token to perform actions like  generate records for compliance reporting, subscription management, and more. 
For more information, see [Create an account-level API Account](https://support.bigcommerce.com/s/article/Store-API-Accounts?language=en_US#account-level).

>>>>Start: I think we should remove this part for now <<<
...
>>>>END: I think we should remove this part for now <<<
...
| API or Use Case | Store-level API Account | Account-level API Account |
|:----------------|:---------------:|:-----------------:|
