...
<Callout type="info">
  When a retailer enters tax-inclusive prices, the `should_subtract_store_tax` field allows you to choose how a store calculates the tax-inclusive price displayed to shoppers. This setting affects the tax-inclusive price displayed to a shopper if the shopper and retailer are in different tax zones. For more information, see the [Tax Settings API Reference](/docs/rest-management/tax-settings). 
</Callout>
