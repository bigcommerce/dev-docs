...
<Callout type="info">
  The `subtract_store_tax_field` field allows you to choose how a store calculates tax-inclusive prices for a shopper when a merchant enters tax-inclusive prices. This setting affects the tax-inclusive price for a shopper if the shopper and merchant are in different tax zones. For more info, see the [Tax Settings API Reference](/docs/rest-management/tax-settings). 
</Callout>
