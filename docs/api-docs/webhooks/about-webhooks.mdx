...
4. Some webhook events triggered during this time are sent to our retry queues to be executed later when the domain per specific `client_id` is no longer blocklisted and once the retry queue time has elapsed.

If a `client_id` uses the same domain as a blocklisted `client_id` but doesn't have a lot of traffic or once a `client_id` is no longer blocklisted, all new webhook requests will be sent as they occur. Event requests sent to the retry queue during a blocklisting period will be delivered according to the retry queue schedule.
