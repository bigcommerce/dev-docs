...
| <sup>+</sup>store/product/inventory/updated| Fires to indicate changes in inventory for a _base product_. For products with variants, you must track by _product_ and change the _product-level_ inventory. For products without variants, the webhook fires regardless of how you track inventory. |
| <sup>++</sup>store/product/inventory/order/updated| Fires to indicate changes in inventory through an order for a _base product_. The webhook fires for products with or without variants, but only when you track by _product_. |
...
> <sup>+</sup>Inventory updates made through the API, marketplace apps, and control panel can all trigger the webhook. In the control panel, you can bulk import product inventories or update inventory by navigating to the **Products > View** tab.
