...
| store/sku/inventory/order/updated | Fires when the inventory is updated, when an order is placed, and when an order is refunded and the inventory returned to stock. This webhook fires based on a store's Inventory settings. |
