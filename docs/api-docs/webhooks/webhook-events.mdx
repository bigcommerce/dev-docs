...
| store/customer/updated | Fires when a customer is updated. Does not currently track changes to the customer address. <br /><br />Tracks changes to customer attributes only if the changes are made through the Control Panel. This triggers the same event `type` and payload as updating a customer; the payload does not include customer attributes. |
