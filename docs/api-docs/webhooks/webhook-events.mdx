...
| store/product/inventory/updated| Fires to indicate changes in inventory for a _base product_. For products with variants, you must track by _product_ and change the _product-level_ inventory. For products without variants, the webhook fires regardless of how you track inventory. You can make the inventory updates through the API, marketplace apps, or control panel. In the control panel, you can use a CSV import or Orders tab. |
| <sup>+</sup>store/product/inventory/order/updated| Fires to indicate changes in inventory for _variants_. The webhook fires only for products with variants. To fire the webhook, you must track by _variant_ and change _variant-level_ inventory. |
...
| store/sku/inventory/updated | Fires to indicate changes in inventory through an order for a _base product_. The webhook fires for products with or without variants, but only when you track by _product_.  |
| <sup>+</sup>store/sku/inventory/order/updated | Fires to indicate changes in inventory through an order for _variants_. The webhook fires only for products with variants, and you must track by variant. |
