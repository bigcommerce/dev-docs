...
- `STORE_TOKEN|STORE_CLIENT`: For instructions on generating a store-level API account, visit [Creating store-level API Accounts](/api-docs/getting-started/api-accounts#creating-store-level-api-accounts).
