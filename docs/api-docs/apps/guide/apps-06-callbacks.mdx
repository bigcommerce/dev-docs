...
```http file="Example request: /load callback from BigCommerce"
...
```http file="Example request: uninstall callback from BigCommerce"
...
```http file="Example request: remove_user callback"
...
```json file="Example: app callback payload" showLineNumbers copy
{
...
| `sub` | string | The path that identifies the store in API requests to `https://api.bigcommerce.com{:http}`; a string of the form `stores/{STORE_HASH}`. |
