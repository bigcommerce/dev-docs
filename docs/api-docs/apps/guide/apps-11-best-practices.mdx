...
BigCommerce responds to apps that exceed the quota with a [429](/api-docs/getting-started/api-status-codes#4xx-client-error) response. When this happens, applications should pause requests until the number of milliseconds in `X-Rate-Limit-Time-Reset-Ms` elapses.
