...
Creating an app also creates an app-level API account. To learn more about app-level API accounts, see the [Guide to API Accounts](/api-docs/getting-started/api-accounts#app-level-api-accounts). To create an app, do the following:
