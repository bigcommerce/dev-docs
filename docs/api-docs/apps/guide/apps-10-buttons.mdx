...
Single-click apps can be installed from outside the BigCommerce control panel. For example, an install button on your company's site that directs the retailer to download your app. This tutorial provides step-by-step instructions for creating an external install button for BigCommerce single-click apps.
...
Your button will link retailers to BigCommerce's install endpoint for your application. Once the retailer clicks the link, they will be prompted to sign in and authorize your application.
