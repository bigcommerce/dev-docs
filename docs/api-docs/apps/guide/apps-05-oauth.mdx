...
| `user.id` | integer | BigCommerce’s unique identifier for the store owner or authorized user. Save this value to identify the user in future requests. |
| `user.username` | string | The username that the initiating user has on file with BigCommerce. |
| `user.email` | string | The email address that the store owner or authorized user has on file with BigCommerce. Save this value for future requests. |
