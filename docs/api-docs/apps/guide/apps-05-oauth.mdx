...
Your app must expose a callback endpoint, `GET /auth{:http}`, that the retailer's store control panel can hit to initiate the code grant authorization flow. For a list of all the callback endpoints your app can expose, both required and optional, see [Single-Click App Callback Handlers](/api-docs/apps/guide/callbacks).
...
1. The retailer initiates installing your app by signing in to their store control panel and doing one of the following: 
  * clicking **Install** in the [App Marketplace](https://www.bigcommerce.com/apps/), or 
  * following a direct installation link.
2. The retailer accepts the app's OAuth scope permissions. The OAuth consent view presented to the retailer requires them to approve all the scopes to install the app; at this time, retailers cannot pick and choose scopes.
3. The retailer's [browser sends a GET request to the app](#receiving-the-auth-callback) server's `GET /auth{:http}` endpoint that contains some of the information necessary to request a unique `access_token` for the store. 
4. The [app sends POST request to BigCommerce](#requesting-the-access_token) to request a unique `access_token` for the store. 
5. BigCommerce responds with either an error or an `access_token` unique to the retailer's store; see [receiving the access_token response](#receiving-the-access_token-response).
...
After your app has an `access_token` for the store, the following events can mark the `access_token` for invalidation the next time the retailer opens the app:
* You change the app's OAuth scopes.
* The retailer's email address changes.

The next time the retailer opens the app in the store control panel, the browser will prompt them to accept the changes. Once they accept, their previous `access_token` will be invalid, and your app will receive a fresh auth callback for their store.  
...
| `scope` | A space-separated list of the OAuth scopes associated with this app's [API account](/api-docs/getting-started/api-accounts#app-level-api-accounts). |
...
To generate an access_token for the retailer's store, send a `POST` request to `https://login.bigcommerce.com/oauth2/token{:http}`. The request body contains a combination of query arguments from the auth callback and credentials from your app profile.
...
| `user.id` | integer | BigCommerce’s unique identifier for the retailer or authorized user. Save this value to identify the user in future requests. |
...
If you do not respond, the retailer will be left looking at a blank screen and will not be able to interact with your app.
