...
> Typically, only [store owners](/api-docs/getting-started/api-accounts#creating-store-level-api-accounts) can create API accounts and `access_token`s for a store. However, when an app is approved to be publicly available for additional stores to install, it can generate `access_token`s *on behalf* of store owners.
...
| `scope` | A space-separated list of the OAuth scopes associated with this app's [API account](/api-docs/getting-started/api-accounts#app-api-accounts). |
