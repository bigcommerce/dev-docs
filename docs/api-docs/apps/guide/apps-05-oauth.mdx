...
<Tabs items={['Example request', 'URL-decoded query parameters']}>
  <Tab>
```http file="Example request: auth callback" showLineNumbers copy
...
```
  ...
  <Tab>
```json file="Example query parameters: auth callback" showLineNumbers copy
{
...
```http file="Example request: Create an access_token" showLineNumbers copy
...
```json file="Example response: Create an access_token" showLineNumbers copy
{
