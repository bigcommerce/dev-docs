...
4. Click **Technical**.
5. Enter the **Auth Callback URL** as `https://{ngrok_id}.ngrok.io/api/auth{:http}`. For example, `https://12345.ngrok.io/api/auth{:http}`. You can get the `ngrok_id` from the terminal that is running `ngrok http 3000{:shell}`.
6. Enter the **Load Callback URL** as `https://{ngrok_id}.ngrok.io/api/load{:http}`.
7. Enter the **Uninstall Callback URL** as `https://{ngrok_id}.ngrok.io/api/uninstall{:http}`.
8. If your app supports multiple users, locate the **App Features** section. Select **Multiple Users**, then enter the **Remove User Callback URL** as `https://{ngrok_id}.ngrok.io/api/remove_user{:http}`.
