...
4. Click **Technical**.
5. Enter Auth Callback URL as `https://{ngrok_id}.ngrok.io/api/auth{:http}`. For example, `https://12345.ngrok.io/api/auth{:http}`. You can get the `ngrok_id` from the terminal that is running `ngrok http 3000{:shell}`.
6. Enter Load Callback URL as `https://{ngrok_id}.ngrok.io/api/load{:http}`.
7. Enter Uninstall Callback URL as `https://{ngrok_id}.ngrok.io/api/uninstall{:http}`.
8. Because you will be modifying the Products API resource, set the Products OAuth scope to **MODIFY**. To learn more about the available OAuth scopes, see [OAuth scopes](/api-docs/getting-started/api-accounts#oauth-scopes).
...
4. Update the `AUTH_CALLBACK` with the `ngrok_id`. You can get the `ngrok_id` from the terminal that is running `ngrok http 3000{:shell}`. 
