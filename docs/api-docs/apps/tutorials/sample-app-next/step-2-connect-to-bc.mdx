...
```shell copy file="Install node client"
...
```ts file="Create BC instance auth.ts" showLineNumbers copy
...
```ts file="Export getBCAuth auth.ts" showLineNumbers copy
...
```ts file="Exports getBCVerify auth.ts" showLineNumbers copy
...
```ts file="Add logic auth.ts" showLineNumbers copy
...
```ts file="Add logic load.ts" showLineNumbers copy
import { NextApiRequest, NextApiResponse } from 'next';
...
```ts file="Add logic uninstall.ts" showLineNumbers copy
import { NextApiRequest, NextApiResponse } from 'next';
import { getBCVerify } from '../../lib/auth';
...
```shell copy file="Install ngrok"
npm install ngrok -g
```

2. Expose the web server on port 3000 to the internet.

```shell copy file="Expose server ngrok"
ngrok http 3000
```
...
```shell copy file="Initial contents .env"
...
```shell copy file="Restart dev server"
npm run dev
```
