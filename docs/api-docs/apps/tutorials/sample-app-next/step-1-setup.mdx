...
```shell copy file="Check Node.js version"
node -v
```
...
```shell copy file="Initialize package.json"
npm init
```
...
```json file="Add npm scripts" showLineNumbers copy
{
  ...
  "scripts": {
    "dev": "next",
...
```tsx file="Add functional component index.tsx" showLineNumbers copy
const Index = () => (
...
```tsx file="Functional component _app.tsx" showLineNumbers copy
...
```ts file="Import components Document and DocumentContext"
import Document, { DocumentContext } from 'next/document';
```

3. Import `ServerStyleSheet` from styled-components.

```ts file="Import component ServerStyleSheet"
...
```tsx file="Extend class Document" showLineNumbers copy
...
```shell copy file="Start development server"
npm run dev
```
