...
POS Foundation provides a proof-of-concept [open source framework](https://github.com/bigcommerce/point-of-sale-foundation) for building point-of-sale (POS) checkout interfaces that serve BigCommerce retailers with physical locations. It provides a template for custom POS solutions that use secure, pre-certified EMV card readers. POS Foundation's default integration with Stripe Terminal can significantly accelerate development time. 
...
2. In the control panel of your sandbox store, [create a store-level API account](/api-docs/getting-started/api-accounts#creating-store-level-api-credentials) and add the following OAuth scopes:
...
* [Create a Store-level API Account](/api-docs/getting-started/api-accounts#creating-store-level-api-credentials)
