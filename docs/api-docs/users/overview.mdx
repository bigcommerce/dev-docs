...
1. Request an access token in **Settings** > **Account API tokens** from the control panel. Note that this is different from a store token or a store-level API account.
