...
* BigCommerce store
* A valid API account access token with sufficient permissions to make the desired requests (see Getting Started below)
...
The following query returns details about an account's users. Note a standard cursor-based pagination is supported:
