...
To display the contents of a cart, we need to send a GET request to the [Get a Cart](/docs/rest-storefront/carts#get-a-cart) endpoint. By default, the cart response returns abbreviated product details. To get the full product details, we need to add the `include` query parameter.
