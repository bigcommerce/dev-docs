...
You are required to include the `channel_id` in the login JWTs to embed checkout for headless storefronts. Default value = 1. For more information, see the [Embedded Checkout Overview](/api-docs/storefronts/embedded-checkout/embedded-checkout-overview).
