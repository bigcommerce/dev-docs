...
With Channels, Sites and Routes APIs, developers can quickly create an app that provides retailers with the ability to manage the full experience of selling a product on both BigCommerce and an external platform, from listing products to receiving orders to shipping. They also provide the ability to build apps that help retailers optimize their selling across other services, such as re-pricers and other related services.

Through these integrations, retailers can easily expand where they sell their product catalog online, helping broaden shopper reach with greater merchandising opportunities.
...
The APIs that you integrate with and how you use them (read only vs. modify) will determine which BigCommerce scopes your app requests when it is installed by a retailer and which endpoints you’ll need to query. 
...
Currently, certain channel apps, including native BigCommerce apps and specific partner apps, can be found in a retailer’s BigCommerce control panel in the “Channel Manager” section. Partners can work with the BigCommerce team to get strategic placement in this section, enabling retailers to see their specific channel first. Additionally, this discovery takes place in the control panel, which is deeply integrated into all retailers’ day to day workflow; whereas those found in the App Marketplace must be found by searching within the App Marketplace, which is not natively found in the control panel.
...
An express installable app means that all scopes and permissions will be automatically granted to the app when the retailer chooses to install it. This enables a much more seamless installation process, as the retailer has to click fewer times to install the app.
