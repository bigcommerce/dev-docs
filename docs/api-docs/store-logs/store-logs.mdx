...
The Store Logs API allows retailers to track events in their store. Each event creates an entry in the log. To process events over time, you can configure external systems to retrieve store log entries. For example, you can see how often retailers request shipping quotes from a carrier. You can also learn more about errors that occur on a store.
...
To see how often retailers request shipping quotes from a specific carrier, URL-encode the carrier name and pass it to the `module` query parameter. 
