...
4. To create a new category using the [Catalog API](/docs/rest-management/catalog), send a `POST` request to [`/v3/catalog/categories`](/api-reference/store-management/catalog/category/createcategory). Use the URL defined in the `config.stencil.json` category mapping. In our example, it is `/custom-widget-templates/`.
