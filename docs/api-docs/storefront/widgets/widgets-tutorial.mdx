...
4. To create a new category using the [Catalog API](/docs/rest-catalog), send a `POST` request to [`/v3/catalog/categories`](/docs/rest-catalog/category#create-a-category). Use the URL defined in the `config.stencil.json` category mapping. In our example, it is `/custom-widget-templates/`.
