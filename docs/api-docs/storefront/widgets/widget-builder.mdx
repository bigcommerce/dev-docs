...
* Store API OAuth credentials for `content` resources with scope `modify`. For information on how to generate store API credentials, see [Creating store-level API credentials](/api-docs/getting-started/api-accounts#creating-store-level-api-credentials).
