...
* A store-level API account for `content` resources with scope `modify`. For information on how to generate store-level API accounts, see [Creating store-level API accounts](/api-docs/getting-started/api-accounts#creating-store-level-api-accounts).
