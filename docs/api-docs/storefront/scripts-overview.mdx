...
After a retailer installs your app, the app can freely update its own scripts. All the app's scripts are available in the Script Manager for the retailer to inspect. 
...
If your app is already live, you may have previously asked retailers to paste a code snippet into one of these places:
...
If you want your retailers to upgrade to programmatically created scripts, you can prompt them to do so on your app’s landing page. You’ll need to keep track of which of your retailers are not using the Scripts API to know whether you need to display this prompt. For more on checking which theme engine a storefront is using, see this article's section on [Scripts API and Script Manager](#scripts-api-and-script-manager).

Once a user decides to upgrade, you can use a form to walk them through removing the old manual code in their control panel. When the retailer clicks submit, install your scripts using the Scripts API.
...
**The retailer deleted your script**

Retailers receive a warning when they attempt to delete a script belonging to an app, but they can.
...
**The retailer's theme is not set up correctly** 
...
Stencil themes from the marketplace support [Optimized One-Page Checkout (Help Center)](https://support.bigcommerce.com/articles/Public/Optimized-Single-Page-Checkout). However, there could be instances where the retailer maintains their private theme without the ability to support the optimized one-page checkout. In this case, the retailer must add the theme support, following the steps to customize [optimized one-page checkout](/stencil-docs/customizing-checkout/optimized-one-page-checkout).  
