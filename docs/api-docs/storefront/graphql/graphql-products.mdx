...
BigCommerce's GraphQL Storefront API lets frontend developers retrieve [products](/docs/rest-catalog/products) from a store. These built-in capabilities allow Stencil developers to customize their hosted and headless storefronts with product information.
