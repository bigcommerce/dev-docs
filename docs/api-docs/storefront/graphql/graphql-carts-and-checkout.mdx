...
To make requests, create a [store-level API account](/docs/start/authentication/api-accounts#store-level-api-accounts) with one or more of the following [token creation OAuth scopes](/api-docs/getting-started/api-accounts#token-creation-scopes):
...
For more information, see [Creating a Token](/api-docs/storefront/graphql/graphql-api-overview#creating-a-token) in the GraphQL Storefront API Overview and [Dynamic Tokens](/docs/start/authentication#bigcommerce-generated-jwts) in the Authentication and Example Requests article.
