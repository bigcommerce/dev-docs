...
Use the [Create a GraphQL Storefront API token](/docs/storefront-auth/tokens#create-a-token) REST endpoint to request JWT-style bearer tokens that authenticate cross-origin requests to the GraphQL Storefront API.

This type of token is most appropriate for use directly within web browsers. If you're creating a token for an application that will make server-to-server requests to the Storefront API, you might be better served by a [Customer Impersonation Token](#customer-impersonation-tokens).
...
Only use the [Revoke a token](/docs/storefront-auth/tokens#revoke-a-token) endpoint to revoke compromised tokens under emergency situations. Let uncompromised short-lived tokens expire naturally, as you do not need to revoke these.
...
```js filename="Example request: GraphQL query using Stencil token" showLineNumbers copy
fetch('/graphql', {
  method: 'POST',
...
To expose a channel's permanent URL, [create a site](/docs/rest-management/sites#create-a-site) for the channel.
