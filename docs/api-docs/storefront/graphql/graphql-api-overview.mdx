...
In addition to the complexity limit, there is a query depth limit. The limit is 16.

```graphql filename="Example query with high query depth" showLineNumbers copy
// Each child category adds an additional level of depth.
...
Filter arguments and character limits do not contribute to complexity.
...
Mutations have higher complexity than queries; you may only send one mutation in a single request. [Pagination info](#pagination) also contributes to complexity.
