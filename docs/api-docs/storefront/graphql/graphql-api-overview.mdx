...
It's also possible to generate tokens for use in server-to-server or headless interactions with a trusted consumer using the [Create a customer impersonation token](/docs/storefront-auth/tokens#customer-impersonation-tokens) endpoint. Consult the endpoint's documentation to determine what OAuth scopes it requires.
...
| Platform           | Limits        |
|:-------------------|:--------------|
| Maximum [banners](/theme-objects/schemas#banners) per query | 50  | 
| Maximum [products](/theme-objects/schemas#products) per query | 50  | 
