...
Use the `isVisibleInNavigation` field to filter for only pages that appear in the storefront's navigation menu. Without the filter, queries also return pages that are hidden from the menu, as these pages are still accessible by URL.

Built-in blogs are global and apply to an entire store. When you query web pages for the default storefront channel, the response includes the main blog index page.
...
For rendered regions on the main blog index page and blog posts, see [Rendered Regions on Blogs](#rendered-regions-1).
...
Queries return blogs even if they are hidden from the storefront's navigation menu, as the main blog index page and blog posts are still accessible by URL.
...
The following example retrieves rendered widgets on the main blog index page and blog posts.
...
You can also retrieve rendered widgets on the main blog index page using the `renderedRegionsByPageType` field, as shown in the following example:
...
The main blog index page and blog posts can be retrieved using the web page URL.

The following query retrieves info for the main blog index page.
