...
Use the `isVisibleInNavigation` field to filter for pages that are not hidden from the storefront's navigation menu. Without the filter, the query also returns pages that are hidden from the menu, as these pages are still accessible by URL.
          ...
          "edges": [
            ...
            },
            {
              "node": {
                ...
                "plainTextSummary": "This is an example contact page!!!",
                "contactFields": [],
                "renderedRegions": {
                  "regions": [
                    {
...
For rendered regions of the blog page and blog posts, see [Blog Rendered Regions](#rendered-regions-1).

### Routes

Plain-text pages and contact form pages can be retrieved using the web page URL.
...
```graphql filename="Example query: Get rendered regions for blog" showLineNumbers copy
query {
  site {
    content {
